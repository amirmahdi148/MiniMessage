...
const socket = io("http://192.168.1.164:5000");
...
const Chatroom = ({ rec, setPage }) => {
  ...
  const verifyUser = async () => {
    try {
      ...
      const response = await fetch(
        "http://192.168.1.164:5000/api/verify",
        {
          method: "POST",
  ...
  useEffect(() => {
    ...
    const fetchReceiverData = async () => {
      try {
        const res = await fetch(
          "http://192.168.1.164:5000/api/getdata",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        setImg(`http://192.168.1.164:5000/uploads/${fixedURL.split('/').pop()}`)
        setIsValid(true);
    ...
    const fetchMessages = async () => {
      try {
        const res = await fetch(
          "http://192.168.1.164:5000/api/messages",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
